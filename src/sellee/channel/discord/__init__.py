...
`ws_client` is a thin wrapper over the `websockets` library's synchronous client; `gateway` is
the Gateway session loop and its off/awaiting-bind/bound states — the Discord analog of
`telegram/poller.py`, since Discord has no long-poll receive API and instead requires a
persistent, outbound-initiated WebSocket connection; `transport` is the REST
